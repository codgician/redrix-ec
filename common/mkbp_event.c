...
static int mkbp_set_host_active_via_gpio(int active, uint32_t *timestamp)
{
	...
	/*
	 * In case EC_INT_L is not a wake pin or the AP doesn't respond to the
	 * GPIO interrupt (e.g. custom Linux), also notify the AP via a host
	 * event so the eSPI SCI path can pick up MKBP events in S0 as well.
	 */
	if (active)
